...
function soloAdmins(req, res, next) {
  if (req.query.rol !== "admin") {
    res
      .status(500)
      .send("Usuario no autorizado para acceder a esta caracteristica");
  }
  next();
}
...
carritosRouter.post("/admin", soloAdmins, async (req, res) => {
  let today = Date.now();
...
//Permite borra un carrito con el id pasado por props
...
carritosRouter.delete("/:idCart/:idProduct/products", async (req, res) => {
  res.json(
    await CartService.deleteObjInCart(
